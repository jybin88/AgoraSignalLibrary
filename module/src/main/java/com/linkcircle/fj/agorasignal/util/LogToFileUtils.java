...
public class LogToFileUtils {
     ...
     * 日志中的时间显示格式
     */
    private static SimpleDateFormat logSDF = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
    /**
     * 日志的最大占用空间 - 单位：字节
     * <p>
...
//            file = new File(mContext.getExternalFilesDir("Log").getPath() + "/");
            ...
            file = new File(sdcardPath + "/sf-express/com.sf.leafnet/log/Xlog/");
        } else {
            // 没有SD卡或者外部存储器，使用内部存储器
            // \data\data\包名\files\Log\logs.txt
//            file = new File(mContext.getFilesDir().getPath() + "/Log/");
            file = new File(mContext.getFilesDir().getPath() + "/sf-express/com.sf.leafnet/log/Xlog/");
        }
        // 若目录不存在则创建目录
        if (!file.exists()) {
            boolean mkdir = file.mkdirs();
            Log.d(MY_TAG, "mkdir: " + mkdir);
        }
